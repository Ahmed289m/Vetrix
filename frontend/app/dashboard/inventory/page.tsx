  ...
  return (
    <motion.div variants={{ animate: { transition: { staggerChildren: 0.06 } } }} initial="initial" animate="animate" className="space-y-6 max-w-6xl mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <motion.div variants={fadeUp} className="grid grid-cols-3 gap-3">
        <div className="glass-card p-4 border border-emerald/20"><p className="text-3xl font-extrabold text-emerald">{items.filter((i) => getStockLevel(i) === "normal").length}</p><p className="text-xs text-muted-foreground mt-1">In Stock</p></div>
        <div className="glass-card p-4 border border-orange/20"><p className="text-3xl font-extrabold text-orange">{lowCount}</p><p className="text-xs text-muted-foreground mt-1">Low Stock</p></div>
        <div className="glass-card p-4 border border-coral/20"><p className="text-3xl font-extrabold text-coral">{criticalCount}</p><p className="text-xs text-muted-foreground mt-1">Critical</p></div>
      </motion.div>
              ...
              {filtered.map((item) => {
                ...
                return (
                  <tr key={item.id} className="border-b border-border/20 hover:bg-muted/10 transition-colors group">
                    <td className="px-5 py-3.5"><p className="font-semibold">{item.name}</p></td>
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <span className={`px-2 py-0.5 rounded-lg text-[10px] font-bold uppercase ${level === "critical" ? "bg-coral/15 text-coral" : level === "low" ? "bg-orange/15 text-orange" : "bg-emerald/15 text-emerald"}`}>{level}</span>

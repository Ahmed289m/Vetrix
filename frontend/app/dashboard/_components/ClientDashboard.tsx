...
export function ClientDashboard() {
  ...
  return (
    <>
      ...
      {/* ── Floating Customer Service Chat (Beta) ── */}
      <AnimatePresence>
        {showChat && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 z-50 w-[min(420px,calc(100vw-2rem))] h-[min(600px,calc(100vh-8rem))] rounded-2xl border border-border/40 bg-background shadow-2xl shadow-black/20 overflow-hidden flex flex-col"
          >
            {/* Chat header with close */}
            <div className="shrink-0 flex items-center justify-between px-4 py-3 border-b border-border/30 bg-card/80 backdrop-blur-xl">
              <div className="flex items-center gap-2">
                <MessageSquareText className="w-4 h-4 text-emerald" />
                <span className="text-sm font-bold">AI Assistant</span>
                <span className="px-1.5 py-0.5 rounded-md bg-amber-500/15 text-amber-500 text-[9px] font-black uppercase tracking-wider border border-amber-500/20">
                  Beta
                </span>
              </div>
              <button
      ...
      <motion.button
        ...
        className={cn(
          ...
          showChat
            ? "bg-muted/80 border border-border/40 text-foreground shadow-md"
            : "gradient-emerald-cyan text-primary-foreground glow-emerald shadow-xl shadow-emerald/25",
        )}
      >
        {showChat ? (
          <X className="w-5 h-5" />
      ...
      {/* Beta label on FAB */}
      {!showChat && (
        <span className="fixed bottom-[4.5rem] right-5 z-50 px-1.5 py-0.5 rounded-md bg-amber-500/15 text-amber-500 text-[8px] font-black uppercase tracking-wider border border-amber-500/20 pointer-events-none">
          Beta
        </span>
      )}
    </>

...
const quickActions = [
  ...
  { title: "New Appt", icon: Plus, gradient: "gradient-emerald-cyan" },
...
const statusColors: Record<string, string> = {
  normal: "border-emerald/30 bg-emerald/5",
  "follow-up": "border-orange/30 bg-orange/5",
  emergency: "border-coral/30 bg-coral/5",
...
const statusDot: Record<string, string> = {
  normal: "bg-emerald",
  "follow-up": "bg-orange",
  emergency: "bg-coral animate-pulse",
  "in-progress": "bg-cyan animate-pulse",
      ...
      >
        <div>
          ...
          <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight font-arabic">
            Welcome back <span className="gradient-text">Dr. Emily</span>
          </h2>
          <p className="text-sm text-muted-foreground mt-1.5">
            <span className="font-semibold text-foreground">4 Surgeries</span>{" "}
            and{" "}
              ...
              {appointments.map((apt, i) => (
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col items-center shrink-0 pt-2">
                    <div
                      className={`w-2.5 h-2.5 rounded-full ${statusDot[apt.status]} ring-2 ring-background`}
                    />
                  </div>
                  <div
                    className={`flex-1 p-3 sm:p-4 rounded-xl border ${statusColors[apt.status]} backdrop-blur-sm group-hover:scale-[1.01] transition-all duration-300`}
                  >
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span
                        className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-lg shrink-0 ${apt.status === "emergency" ? "bg-coral/20 text-coral" : apt.status === "in-progress" ? "bg-cyan/20 text-cyan" : apt.status === "completed" ? "bg-muted text-muted-foreground" : "bg-emerald/20 text-emerald"}`}
                      >
                        {apt.status}
                      </span>
                    </div>

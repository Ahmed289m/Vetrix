  ...
  return (
    <motion.div variants={stagger} initial="initial" animate="animate" className="space-y-5 sm:space-y-6 max-w-7xl mx-auto">
      <motion.div variants={fadeUp} className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          <p className="text-xs font-semibold text-emerald uppercase tracking-widest mb-1">Reception Desk</p>
          <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight font-arabic">Good Morning, <span className="gradient-text">Sarah</span></h2>
          <p className="text-sm text-muted-foreground mt-1.5"><span className="font-semibold text-foreground">14 appointments</span> today · <span className="font-semibold text-foreground">2 doctors</span> on duty</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <motion.button whileTap={{ scale: 0.95 }} onClick={() => setShowSim(!showSim)}
            ...
            <Play className="w-3.5 h-3.5" /> {showSim ? "Exit Sim" : "Simulation"}
          </motion.button>
          <motion.button whileHover={{ y: -1 }} whileTap={{ scale: 0.97 }} className="flex items-center gap-2 gradient-emerald-cyan text-primary-foreground px-4 sm:px-5 py-3 rounded-xl text-sm font-bold glow-emerald ripple">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6">
        <motion.div variants={fadeUp} className="xl:col-span-2">
          <div className="glass-card p-5 border-glow text-card-foreground">
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2"><Clock className="w-4 h-4 text-emerald" /><h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Today's Schedule</h3></div>
              <span className="text-xs font-bold text-emerald">{appointments.length} Total</span>
            </div>
            <div className="relative space-y-0">
              ...
              {appointments.map((apt, i) => (
                <motion.div key={apt.time} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}
                  ...
                  <span className="text-[11px] sm:text-sm font-mono tabular-nums text-muted-foreground w-9 sm:w-12 shrink-0 pt-3 text-right">{apt.time}</span>
                  <div className="relative z-10 flex flex-col items-center shrink-0 pt-3">
                    <div className={`w-2.5 h-2.5 rounded-full ring-2 ring-background ${apt.status === "checked-in" ? "bg-emerald" : apt.status === "waiting" ? "bg-orange animate-pulse" : "bg-muted-foreground/30"}`} />
                  </div>
                  <div className={`flex-1 p-3 sm:p-4 rounded-xl border backdrop-blur-sm transition-all duration-300 group-hover:scale-[1.01] ${apt.status === "checked-in" ? "border-emerald/20 bg-emerald/5" : apt.status === "waiting" ? "border-orange/20 bg-orange/5" : "border-border/50 bg-muted/10"}`}>
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {apt.status === "checked-in" ? (
                        ...
                        <button className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-muted/40 text-muted-foreground hover:bg-emerald hover:text-primary-foreground transition-all">Check In</button>
                      )}
                    </div>
                  </div>
                </motion.div>
        ...
        <motion.div variants={fadeUp} className="space-y-6">
          <div className="glass-card p-5 text-card-foreground">
            <div className="flex items-center justify-between mb-4"><h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Weekly Revenue</h3><span className="flex items-center gap-1 text-xs font-bold text-emerald"><TrendingUp className="w-3.5 h-3.5" /> +18%</span></div>
          ...
          <div className="glass-card p-5 text-card-foreground">
            <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-4">Billing Queue</h3>
          ...
          <div className="glass-card p-5 text-card-foreground">
            <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-4 flex items-center gap-2"><AlertTriangle className="w-3.5 h-3.5 text-coral" />Inventory Alerts</h3>
            <div className="space-y-2.5">
              {inventoryAlerts.map(item => (
                <div key={item.item} className={`p-3 rounded-xl border ${item.level === "critical" ? "border-coral/20 bg-coral/5" : "border-orange/20 bg-orange/5"}`}>
                  ...
                  <p className="text-xs text-muted-foreground mt-1.5">Stock: <span className="font-mono font-semibold">{item.stock}</span></p>
                </div>
              ))}
            </div>
          </div>

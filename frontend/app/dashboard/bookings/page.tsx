...
const initialBookings: Booking[] = [
  { id: "1", petName: "Bella", species: "dog", ownerName: "Sarah Mitchell", date: "2024-03-18", time: "09:00", type: "Post-op Follow-up", doctor: "Dr. Emily Chen", status: "pending" },
  { id: "2", petName: "Max", species: "cat", ownerName: "Tom Parker", date: "2024-03-18", time: "09:30", type: "Dental Cleaning", doctor: "Dr. Emily Chen", status: "confirmed" },
  { id: "3", petName: "Luna", species: "dog", ownerName: "Maria Garcia", date: "2024-03-18", time: "10:00", type: "Vaccination", doctor: "Dr. Aris Rahman", status: "pending" },
  { id: "4", petName: "Rocky", species: "dog", ownerName: "James Wilson", date: "2024-03-18", time: "10:30", type: "Skin Allergy Consult", doctor: "Dr. Emily Chen", status: "confirmed" },
  { id: "5", petName: "Mittens", species: "cat", ownerName: "Amy Chen", date: "2024-03-18", time: "11:00", type: "Annual Checkup", doctor: "Dr. Aris Rahman", status: "pending" },
  { id: "6", petName: "Coco", species: "dog", ownerName: "David Lee", date: "2024-03-19", time: "09:00", type: "Blood Work", doctor: "Dr. Emily Chen", status: "pending" },
  { id: "7", petName: "Shadow", species: "cat", ownerName: "Lisa Brown", date: "2024-03-19", time: "10:00", type: "Spay Surgery", doctor: "Dr. Aris Rahman", status: "confirmed" },
  { id: "8", petName: "Duke", species: "dog", ownerName: "Mike Johnson", date: "2024-03-17", time: "14:00", type: "Ear Infection", doctor: "Dr. Emily Chen", status: "completed" },
];

const statusColors: Record<string, { bg: string; text: string; border: string }> = {
  pending: { bg: "bg-orange/10", text: "text-orange", border: "border-orange/20" },
  confirmed: { bg: "bg-emerald/10", text: "text-emerald", border: "border-emerald/20" },
  cancelled: { bg: "bg-coral/10", text: "text-coral", border: "border-coral/20" },
  completed: { bg: "bg-muted/30", text: "text-muted-foreground", border: "border-border/30" },
};
...
export default function BookingsPage() {
  ...
  const handleConfirm = (id: string) => setBookings((prev) => prev.map((b) => b.id === id ? { ...b, status: "confirmed" as const } : b));
  const handleCancel = (id: string) => setBookings((prev) => prev.map((b) => b.id === id ? { ...b, status: "cancelled" as const } : b));
              ...
              <div className="flex items-center gap-4 flex-1 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-bold truncate">{booking.petName}</p>
                    <span className={`px-2 py-0.5 rounded-lg text-[10px] font-bold uppercase ${sc.bg} ${sc.text}`}>{booking.status}</span>
                  </div>
                  <p className="text-xs text-muted-foreground truncate">{booking.ownerName} · {booking.type}</p>
                </div>
              </div>

  ...
  return (
    <div className="min-h-svh bg-background text-foreground relative overflow-hidden flex flex-col">
      ...
      <nav className="relative z-10 px-6 py-6 flex items-center justify-between max-w-7xl mx-auto w-full">
        ...
        <Link 
          href="/login" 
          className="px-5 py-2.5 rounded-xl bg-white/5 border border-white/10 text-sm font-bold hover:bg-white/10 transition-all"
        >
          Sign In
        </Link>
      </nav>
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center px-6 text-center max-w-4xl mx-auto">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald/10 border border-emerald/20 mb-6">
            ...
            <span className="text-[10px] font-bold uppercase tracking-wider text-emerald">Now in Private Beta</span>
          </div>
          ...
          <p className="text-base sm:text-xl text-muted-foreground mb-10 max-w-2xl mx-auto leading-relaxed">
            Vetrix is the next-generation clinic management platform powered by AI. 
            Streamline your workflow, automate records, and focus on what matters most—saving lives.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 w-full sm:w-auto">
            ...
            <button className="w-full sm:w-auto px-8 py-4 rounded-2xl bg-white/5 border border-white/10 font-bold hover:bg-white/10 transition-all">
              Watch Demo
            </button>
          </div>
        </motion.div>
        ...
        >
          {[
            { icon: Zap, title: "Lighting Fast", desc: "Built for speed and zero latency operations." },
            { icon: Shield, title: "Clinic Secure", desc: "Enterprise-grade encryption for patient data." },
            { icon: Sparkles, title: "AI Powered", desc: "Automated diagnostics and clinical insights." },
          ].map((f, i) => (
      ...
      <footer className="relative z-10 py-10 px-6 border-t border-border/5 mt-20">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-6">
          <p className="text-xs text-muted-foreground/40 font-medium">© 2024 Vetrix Health Technologies. All rights reserved.</p>
          <div className="flex items-center gap-6">
            <a href="#" className="text-xs text-muted-foreground/40 hover:text-emerald transition-colors">Privacy</a>
            <a href="#" className="text-xs text-muted-foreground/40 hover:text-emerald transition-colors">Terms</a>
            <a href="#" className="text-xs text-muted-foreground/40 hover:text-emerald transition-colors">Support</a>
          </div>
        </div>
      </footer>
    </div>

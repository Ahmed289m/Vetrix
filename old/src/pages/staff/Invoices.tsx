...
import { FileText, Plus, Download, Eye, X, Printer, CheckCircle2 } from "lucide-react";
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} onClick={e => e.stopPropagation()} className="glass-card p-6 w-full max-w-2xl max-h-[80vh] overflow-y-auto custom-scrollbar space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-muted/20 border border-border/30">
                <div>
                  ...
                  <p className="text-sm font-semibold mt-0.5">{selectedInvoice.ownerName}</p>
                </div>
                <div>
                  <p className="text-[10px] font-bold uppercase text-muted-foreground">Patient</p>
                  <p className="text-sm font-semibold mt-0.5">{selectedInvoice.petName}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold gradient-emerald-cyan text-primary-foreground glow-emerald">
